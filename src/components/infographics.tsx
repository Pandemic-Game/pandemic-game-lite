...
// Public support
export function SupportBar(props: { indicators: Indicators, response: Response, onClickSource: Function, delay: number }) {
    const responseText = props.response.updatedIndicators.supportForLastResponse > 50 ? 
        'People liked that!' : 
        'Ooh... People found that controversial!'
    ;
    const barWidth = {
        support: Math.max(props.response.updatedIndicators.supportForLastResponse, 20),
        opposition: Math.max(props.response.updatedIndicators.oppositionToLastResponse, 20)
    };
    return (
        <div 
        ...
        >
            ...
            <div className='w-full p-2 flex flex-row justify-center items-center'>
                ...
                >
                    ...
                    <Txt.Subtitle value={`${props.indicators.supportForLastResponse}%`} col='black' />
                </div>
                <div 
                ...
                >
                    ...
                    <Txt.Subtitle value={`${props.indicators.oppositionToLastResponse}%`} col='black' />
                </div>
            </div>
        </div>
    );
...
function CaseCircles(props: { thisTurn: Indicators, lastTurn: Indicators, delay: number }) {
    ...
    if (casesAreIncreasing) { // If cases increasing
        ...
        for (let i = 0; i < Math.max(0, props.thisTurn.newCases - props.lastTurn.newCases); i++) {
            circles.push(<CaseCircle key={`increase-${i}`} type='increase' animationDelay={
                props.delay + ( i / (props.thisTurn.newCases - props.lastTurn.newCases) * 3 )
            } />)
        }
        ...
        for (let i = 0; i < Math.max(0, props.lastTurn.newCases - props.thisTurn.newCases); i++) {
            circles.push(<CaseCircle key={`decrease-${i}`} type='decrease' animationDelay={
                props.delay + (( props.lastTurn.newCases - props.thisTurn.newCases - i) / props.lastTurn.newCases * 3)
            } />)
        }
    }
...
export function CaseGraphic(props: { thisTurn: Indicators, lastTurn: Indicators, delay: number  }) {
...
// Economy
export function EconomyGraphic(props: { indicators: Indicators, delay: number  }) {
        ...
        >
            ...
            <div className='flex flex-row'>
                ...
                <div className='mb-4 flex flex-col items-center'>
                    ...
                    <div className=''>
                        <Txt.Text value={`${props.indicators.lockdownCosts === 0 ? 'Very little or none' : `$${props.indicators.lockdownCosts}Billion`} `} col='white' />
                    </div>
                    <img
                        src={lockdownCoin}
