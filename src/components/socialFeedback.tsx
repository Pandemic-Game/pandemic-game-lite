...
export function Tweet(props: {fb: ResponseItem, animation: string}){
    return (
        <div className={`w-auto p-3 m-2 flex flex-col justify-center items-start relative bg-white rounded-xl ${props.animation}`}>
...
export function News(props: {fb: ResponseItem, animation: string}){
    return (
        <div className={`w-auto p-3 m-2 flex flex-col justify-center items-start relative bg-white rounded-xl ${props.animation}`}>
            ...
            <p className='text-left text-2xl font-medium' style={{fontFamily: 'Playfair Display'}}>{props.fb.content}</p>
...
export function Meme(props: {fb: ResponseItem, animation: string}){
    return (
        <div className={`w-auto p-3 m-2 flex flex-col justify-center items-center relative bg-white rounded-xl ${props.animation}`}>
            <div className='absolute -top-2 -left-2 rounded-full bg-white'>
                <FontAwesomeIcon 
                    icon={props.fb.isHappy ? faLaughBeam : faAngry} 
                    color={props.fb.isHappy ? 'green' : 'red'} 
            ...
            <p className='p-2 text-light text-xl' style={{fontFamily: 'Impact'}}>{props.fb.head}</p>
            <img 
